...
var DecDetail = function(){
    ...
    var listComClick = function () {
        $(".allList").find("li").unbind("click").on("click", function (event) {
            ...
            if (!$(this).hasClass("hang-more")) {
                ...
                /*委托*/
                if ($(this).parents(".allList").attr("id") == "compWt") {
                    realWt = $(this).text();
                    ...
                    if ($(this).index() == 0) {
                        ...
                        $(".listThree").remove();
                        $("#allListSo").find("ul").prepend("<li class='listThree'name =" + proCode + ">" + realWt + "<span class='soListClose'></span></li>");
                    }
                }
                initTable();
                removeThing();
    ...
    var getList = function (data) {
        ...
        $(list).each(function (i) {
            ...
            tr += "<td>" + list[i].serviceType + "</td>";
            tr += "<td class='queryWidthCom'>" + list[i].declareMoney + "</td>";
            
            
            /*tr += "<td>" + list[i].declarePrice + "</td>";*/
            if(Number(list[i].declare) == 0){
            	if(list[i].declareArrow == 0){
            		tr += "<td class='queryWidthCom priceBgGreen'><span class='priceGreen'>" + list[i].declarePrice + "</span></td>";
            	}else if(list[i].declareArrow == 1){
            		tr += "<td class='queryWidthCom priceBgRed'><span class='priceGreen'>" + list[i].declarePrice + "</span></td>";
            	}else{
            		tr += "<td class='queryWidthCom'><span class='priceGreen'>" + list[i].declarePrice + "</span></td>";
            	}
            	
            }else if(Number(list[i].declare) == 1){
            	if(list[i].declareArrow == 0){
            		tr += "<td class='queryWidthCom priceBgGreen'><span class='priceRed'>" + list[i].declarePrice + "</span></td>";
            	}else if(list[i].declareArrow == 1){
            		tr += "<td class='queryWidthCom priceBgRed'><span class='priceRed'>" + list[i].declarePrice + "</span></td>";
            	}else{
            		tr += "<td class='queryWidthCom'><span class='priceRed'>" + list[i].declarePrice + "</span></td>";
            	}
            	
            }else{
            	tr += "<td class='queryWidthCom'>" + list[i].declarePrice + "</td>";
            }
            
            tr += "<td class='queryWidthCom'>" + list[i].declareVolumn + "</td>";
    ...
    var timeIntervalInit = function(){
    	...
    	var page = Query.getHash("page");
    	if(isNullOrEmpty(page)||page == 1){
    ...
    var initTable = function (re) {
        ...
        //特征
        if ($(".listThree").size()) {
            realWt = $(".listThree").attr("name");
        } else if ($("#compWt").find(".hang-active").text() == "全部") {
            realWt = "";
        } else {
            realWt = $("#compWt").find(".hang-active").attr("name");
        }
        //公司标签
        if ($(".listFive").size()) {
        ...
        _url = $.kf.GETAGREEMENTINFO + "?"+"tag=" + tag+ "&type=" + realLx + "&special=" + realQs + "&serviceType=" + realWt +  "&keyword=" + keyword;
        if(re==1){
//      	$.kf.ajax({
//	            type: "get",
//	            url: _url,
...
//	        });
			new GetTableQuotes(_url, $("#pageToolaaa"), "", getList, "get", $("#tableEnd"),lastPage,$(".noLoading")).init();
        }else{
    ...
    var keyWord = function () {
        ...
        $("#soClear").on("click", function () {
            ...
            $("#compWt").find("li").eq(0).addClass("hang-active").siblings().removeClass("hang-active");
